//Задать температуру в градусах по Цельсию. Вывести в alert соответствующую температуру
// в градусах по Фаренгейту. Подсказка: расчёт идёт по формуле: Tf = (9 / 5) * Tc + 32,
// где Tf – температура по Фаренгейту, Tc – температура по Цельсию

const Tc = prompt('Введите градусы по Цельсию: ')
const Tf = (9/5) * Tc + 32
alert(`${Tc} Цельсия = ${Tf} Фаренгейта`)

//Объявить две переменные: admin и name. Записать в name строку "Василий";
// Скопировать значение из name в admin. Вывести admin (должно вывести «Василий»).

let name = 'Василий'
let admin
admin = name
document.write(admin)

// Чему будет равно JS-выражение 1000 + "108"

document.write(1000 + '108') // = 1000108

// Самостоятельно разобраться с атрибутами тега script (async и defer)

// ASYNC - параллельная загрузка страницыи выполнение скрипта
// DEFER - указывает на то, что скрипт должен быть загружен толькопосле того, как HTML-документ будет полностью
// разобран ("отложенная загрузка").
